...
option_market = st.sidebar.selectbox("Option Market", ['CCA','UKA','EUA'], index=['CCA','UKA','EUA'].index(st.session_state.option_market))
...
close_days = st.sidebar.number_input("Days until close", min_value=0, max_value=3650, value=st.session_state.close_days)
...
for i in range(int(num_legs)):
    ...
    with st.sidebar.expander(f"Leg {i+1}"):
        ...
        month = st.selectbox(f"Expiry Month {i+1}", MONTHS, index=0, key=f'options_month_{i}')
        year_sel = st.selectbox(f"Expiry Year {i+1}", ['2025','2026'], index=0, key=f'options_year_{i}')
        ...
        try:
            ...
            if not df_options_source.empty:
                opt_type_col = df_options_source.get('Opt Type').astype(str).str.strip().str.upper()
                month_col = df_options_source.get('Month').astype(str).str.strip()
                year_col = df_options_source.get('Year').astype(str).str.strip().str[-2:]
                ...
                mask = (opt_type_col.str.startswith(opt_code_norm)) & \
                       (month_col == month_norm) & \
                       (year_col == year_norm) & \
                       (strike_col.notna() & np.isclose(strike_col.values, strike_val, atol=1e-6))
...
show_surface = st.sidebar.checkbox("Show surface", value=st.session_state.show_surface)
...
show_greeks = st.sidebar.checkbox("Show greeks", value=st.session_state.show_greeks)
...
use_expected_spot = st.sidebar.checkbox("Use expected spot", value=st.session_state.use_expected_spot)
...
for leg in legs:
    T_close_leg = max(0.0, (leg.get('days', st.session_state.current_days) - st.session_state.close_days) / 365.0)
    prices_close = bs_price_vectorized(S_range, leg['K'], T_close_leg, r, leg['vol'], leg['type'])
...
fig2.add_trace(go.Scatter(x=[spot, spot], y=[y_min, y_max], mode='lines', name=f'Spot = {spot:.2f}',
                          line=dict(color='gray', dash='dot')))
fig2.add_trace(go.Scatter(x=[st.session_state.expected_spot, st.session_state.expected_spot], y=[y_min, y_max], mode='lines', name=f'Expected Spot = {st.session_state.expected_spot:.2f}',
...
for leg in legs:
    prices_t = np.array([bs_price(spot, leg['K'], max(0.0, (leg.get('days', st.session_state.current_days) - int(d))/365.0), r, leg['vol'], leg['type']) for d in days_forward])
    wr_time_total_current += weighted_return_percent(prices_t, leg['entry'], leg['mult'], fx_rate, fund_nav, leg['sign'])

today = pd.to_datetime('today').normalize()
dates_str = (today + pd.to_timedelta(days_forward, unit='D')).strftime('%Y-%m-%d')

fig_time_current = go.Figure()
fig_time_current.add_trace(go.Scatter(x=days_forward, y=wr_time_total_current, mode='lines+markers', name='Weighted return (%NAV)',
...
if 0 <= st.session_state.close_days <= overall_max_days:
    ...
    y_min_c, y_max_c = (-1.0,1.0) if finite_vals.size==0 else (float(np.nanmin(finite_vals)-0.05*(np.nanmax(finite_vals)-np.nanmin(finite_vals)))), float(np.nanmax(finite_vals)+0.05*(np.nanmax(finite_vals)-np.nanmin(finite_vals)))
    fig_time_current.add_shape(type='line', x0=st.session_state.close_days, x1=st.session_state.close_days, y0=y_min_c, y1=y_max_c, line=dict(color='black', dash='dash'))
    fig_time_current.add_annotation(x=st.session_state.close_days, y=y_max_c, text=f'Close in {st.session_state.close_days}d', showarrow=False, yanchor='bottom')

fig_time_current.update_layout(xaxis_title='Days', yaxis_title='Weighted Return (% of NAV)', height=420)
...
for leg in legs:
    prices_t_e = np.array([bs_price(st.session_state.expected_spot, leg['K'], max(0.0, (leg.get('days', st.session_state.current_days) - int(d))/365.0), r, leg['vol'], leg['type']) for d in days_forward_expected])
    wr_time_total_expected += weighted_return_percent(prices_t_e, leg['entry'], leg['mult'], fx_rate, fund_nav, leg['sign'])

dates_str_e = (today + pd.to_timedelta(days_forward_expected, unit='D')).strftime('%Y-%m-%d')

fig_time_expected = go.Figure()
fig_time_expected.add_trace(go.Scatter(x=days_forward_expected, y=wr_time_total_expected, mode='lines+markers', name='Weighted return (%NAV)',
                              customdata=dates_str_e, hovertemplate='Date %{customdata}<br>Weighted return: %{y:.6f}%<extra></extra>'))
if 0 <= st.session_state.close_days <= overall_max_days:
    finite_vals_e = wr_time_total_expected[~np.isnan(wr_time_total_expected)]
    y_min_e, y_max_e = (-1.0,1.0) if finite_vals_e.size==0 else (float(np.nanmin(finite_vals_e)-0.05*(np.nanmax(finite_vals_e)-np.nanmin(finite_vals_e)))), float(np.nanmax(finite_vals_e)+0.05*(np.nanmax(finite_vals_e)-np.nanmin(finite_vals_e)))
    fig_time_expected.add_shape(type='line', x0=st.session_state.close_days, x1=st.session_state.close_days, y0=y_min_e, y1=y_max_e, line=dict(color='black', dash='dash'))
    fig_time_expected.add_annotation(x=st.session_state.close_days, y=y_max_e, text=f'Close in {st.session_state.close_days}d', showarrow=False, yanchor='bottom')

fig_time_expected.update_layout(xaxis_title='Days', yaxis_title='Weighted Return (% of NAV)', height=420)
...
if st.session_state.get('show_greeks', True):
    spot_for_greeks = st.session_state.expected_spot if st.session_state.get('use_expected_spot', False) else st.session_state.get('spot', 110.0)
